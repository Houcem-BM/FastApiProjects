...
async function fetchStudents() {
    ...
    const studentsAsObjects = students.map(([id, name, grade]) => ({
      id:id,
      name:name,
      grade:grade,
    }));
    ...
    studentsAsObjects.forEach(student => {
        tbody.innerHTML += `<tr>
                <td>${student.id}</td>
                <td>${student.name}</td>
                <td>${student.grade}</td>
...
/* two mothods to make update button disappear for each user
<button  onclick = "updateStudent(${student.id},'${student.name}',${student.grade}, this)" >Update </button>  
<button  id="update-${student.id}"  onclick = "updateStudent(${student.id},'${student.name}',${student.grade})" >Update </button>  
*/
